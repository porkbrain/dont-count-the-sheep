...
pub fn system_extract_pipeline_assets<T: LightScene>(
    ...
    *gpu_target_sizes = **res_target_sizes;
                    ...
                    OmniLightSource2D {
                        intensity: light_source.intensity
                            + rng.gen_range(-1.0..1.0)
                                * light_source.jitter_intensity,
                        ..*light_source
                    },
                    Vec2::new(
                        transform.translation().x
                            + rng.gen_range(-1.0..1.0)
                                * light_source.jitter_translation,
...

    {
        ...
        for (occluder, transform, hviz, vviz) in query_occluders.iter() {
            if hviz.get() && vviz.get() {
                ...
                light_occluders
                    .data
                    .push(GpuLightOccluder2D::new(transform, occluder.h_size));
            }
        }
    }
...

    {
        ...
        light_pass_params.probe_atlas_cols =
            gpu_target_sizes.probe_grid_isize.x;
        light_pass_params.probe_atlas_rows =
            gpu_target_sizes.probe_grid_isize.y;
        light_pass_params.reservoir_size = light_pass_config.reservoir_size;
        light_pass_params.smooth_kernel_size_h =
            light_pass_config.smooth_kernel_size.0;
        light_pass_params.smooth_kernel_size_w =
            light_pass_config.smooth_kernel_size.1;
        light_pass_params.direct_light_contrib =
            light_pass_config.direct_light_contrib;
        light_pass_params.indirect_light_contrib =
            light_pass_config.indirect_light_contrib;
        light_pass_params.indirect_rays_radius_factor =
            light_pass_config.indirect_rays_radius_factor;
        light_pass_params.indirect_rays_per_sample =
            light_pass_config.indirect_rays_per_sample;
    }

    {
        ...
        for new_gi_state in query_skylight_light.iter() {
            light_pass_params.skylight_color.x +=
                new_gi_state.color.r() * new_gi_state.intensity;
            light_pass_params.skylight_color.y +=
                new_gi_state.color.g() * new_gi_state.intensity;
            light_pass_params.skylight_color.z +=
                new_gi_state.color.b() * new_gi_state.intensity;
        }
    }

    *gpu_frame_counter = (*gpu_frame_counter + 1)
        % (GI_SCREEN_PROBE_SIZE * GI_SCREEN_PROBE_SIZE);
}
