...
/// The implementation of this trait is autogenerated by the map maker when the
/// map is exported.
///
/// It's stored in `scenes/X/src/autogen/local_tile_impl.rs`.
pub trait ZoneTile {
...
impl<L: ZoneTile<Successors = L>> ZoneTile for TileKind<L> {
    #[inline]
    fn zone_group(&self) -> Option<ZoneGroup> {
        match self {
    ...
    fn zone_successors(&self) -> Option<&'static [Self::Successors]> {
        match self {
            Self::Local(l) => l.zone_successors(),
            _ => None,
        }
    }
}
